...
def eval_defmod(args, summary):
    ...
    for sub, ref in zip(submission, reference):
        ...
        sub["sense-BLEU"] = sentence_bleu([sub["gloss"]], ref["gloss"], smoothing_function=f.method4)
    ...
    for sub in tqdm.tqdm(submission, desc="L-BLEU", disable=None):
        sub["lemma-BLEU"] = max(
            sentence_bleu([sub["gloss"]], g, smoothing_function=f.method4)
    ...
    # 3. write results.
    if args.prefix:
        output_file = f'scores/{args.prefix}-{args.output_file}-{args.lang}-{args.source_arch}.txt'
    else:
        output_file = f'scores/{args.output_file}-{args.lang}-{args.source_arch}.txt'

    print(f"MoverScore_{summary.lang}:{moverscore_average}")
    print(f"BLEU_lemma_{summary.lang}:{lemma_bleu_average}")
    print(f"BLEU_sense_{summary.lang}:{sense_bleu_average}")
...
def eval_revdict(args, summary):
    ...
    output_file = f'{args.output_file}-{args.lang}-{args.source_arch}.txt'
    with open(output_file, "w") as ostr:
        for arch in vec_archs:
...
def main(args):
    def do_score(submission_file, summary):
        ...
        args.reference_file = (
            ...
            / f"{summary.lang}.trial.complete.json"
        )
    ...
    if args.submission_path.is_dir():
        ...
        if args.prefix:
            submission_path = pathlib.Path(f'predictions/{args.submission_path}-{args.prefix}-{args.lang}-{args.source_arch}.json')
        else:
            submission_path = pathlib.Path(f'predictions/{args.submission_path}-{args.lang}-{args.source_arch}.json')
        summary = check_output.main(submission_path)
        do_score(submission_path, summary)
