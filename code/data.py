...
class JSONDataset(Dataset):
    ...
    def __init__(self, file, vocab=None, freeze_vocab=False, maxlen=256, spm_model_name=None):
        """
        ...
        for json_dict in self.items:
            ...
            if "gloss" in json_dict:
                if spm_model_name:
                    json_dict["gloss_tensor"] = self.spm_model.encode(
                            json_dict["gloss"], add_eos=True, add_bos=True
                        )
                else:
                    json_dict["gloss_tensor"] = (
                        [bos]
                        + [
                            self.vocab[word]
                            if not freeze_vocab
                ...
                json_dict["gloss_tensor"] += (maxlen - len(json_dict["gloss_tensor"])) * [self.vocab[PAD]]
                json_dict["gloss_tensor"] = torch.tensor(json_dict["gloss_tensor"])
...
def get_dataloader(dataset, batch_size=32, shuffle=True):
    ...
    def do_collate(json_dicts):
        ...
        if has_gloss:
            ...
            # batch["gloss_tensor"] = torch.stack(batch["gloss_tensor"])
        if has_vecs:
            for arch in SUPPORTED_ARCHS:
